...
scene.add(crystal);
const ball = new THREE.Mesh(new THREE.SphereGeometry(0.3, 16, 8), ballMaterial3);
ball.position.z = -1.5;
ball.position.y = -0.5;
...
const onKeyDown = function (event) {
    switch (event.keyCode) {
        ...
        case 32:
            ...
            const dragControls = new DragControls([crystal], camera, renderer.domElement);
...
crystalFolder.addColor(crystalData, 'color').onChange(() => {
    ballMaterial2.color.setHex(Number(crystalData.color.toString().replace('#', '0x')));
});
